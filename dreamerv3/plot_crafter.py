...
def main():
    ...
    parser.add_argument(
        "--filename", default=default_filename, help="The path to stats.jsonl file"
    )
    ...
    if len(rewards) < 100:
        ...
        plt.plot(
            np.cumsum(lengths).tolist(),
            pd.Series(rewards).rolling(window=100).mean().values,
        )
        plt.xlabel("Steps")
        plt.ylabel("Reward")
        plt.show()
...
def print_summary(runs, budget, verbose):
    ...
    percents, methods, seeds, tasks = compute_success_rates(runs, budget, sortby=0)
