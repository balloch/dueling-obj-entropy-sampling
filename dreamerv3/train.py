...
def make_replay(config, directory=None, is_eval=False, rate_limit=False, **kwargs):
    ...
    if config.replay == "uniform" or is_eval:
        ...
        replay = embodied.replay.CuriousReplay(
            length, size, directory, hyper=config.replay_hyper
        )
    elif config.replay == "does":
        replay = embodied.replay.DisagreementReplay(
            length, size, directory, hyper=config.replay_hyper
        )
    elif config.replay == "per":
        replay = embodied.replay.PrioritizedExperienceReplay(
            length, size, directory, hyper=config.replay_hyper
        )
    elif config.replay == "count-based":
        replay = embodied.replay.CountBasedReplay(
            length, size, directory, hyper=config.replay_hyper
        )
    elif config.replay == "adversarial":
        replay = embodied.replay.AdversarialReplay(
            length, size, directory, hyper=config.replay_hyper
        )
